"""
API routes for KEM cutting process feedback.
"""
from flask import Blueprint, request, jsonify, send_file
from flask import current_app
from models.schemas import KEMFormSchema
...
bp = Blueprint('kem', __name__)
schema = KEMFormSchema()
    ...
    try:
        ...
        if 'data' in request.form:
            ...
            form_data = {
                ...
                'erst_machine_no': request.form.get('erst_machine_no', ''),
        ...
        photo_paths = FileService.save_files(valid_files, 'kem', date_time_str)
        ...
        # Format email body
        email_body = EmailService.format_email_body_kem(form_data)
        ...
        excel_data = {
            ...
            'ERST Machine No': form_data['erst_machine_no'],
            ...
            'Judgement Block': form_data['judgement'],
        ...
        # Save to Excel
        success = ExcelService.append_to_history('kem', excel_data)
        if not success:
        ...
        logger.error(f"Error in KEM submit: {e}", exc_info=True)
        return jsonify({'error': 'Internal server error', 'message': str(e)}), 500


@bp.route('/history', methods=['GET'])
def get_history():
    """Get KEM cutting process feedback history."""
    try:
        history = ExcelService.get_history_as_dict('kem')
        return jsonify(history), 200
    except Exception as e:
        logger.error(f"Error getting KEM history: {e}", exc_info=True)
        return jsonify({'error': 'Failed to retrieve history', 'message': str(e)}), 500


@bp.route('/history/download', methods=['GET'])
def download_history():
    """Download KEM cutting process feedback history as Excel file."""
    try:
        excel_bytes = ExcelService.get_history_excel_bytes('kem')
        return send_file(
            io.BytesIO(excel_bytes),
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            as_attachment=True,
            download_name='history_KEM.xlsx'
        )
    except Exception as e:
        logger.error(f"Error downloading KEM history: {e}", exc_info=True)
