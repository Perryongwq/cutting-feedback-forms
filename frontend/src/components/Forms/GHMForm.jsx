...
import { submitGHMForm, getGHMHistory } from '../../services/api';
...
const GHMForm = () => {
  ...
  const defectOptions = [
    ...
    'Rough cut', 'VP dent', 'Step Shift', 'Ridge/Dent', 'Sample', 'Drop Chips'
  ];
  ...
  const loadHistory = async () => {
    try {
      const response = await getGHMHistory();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await submitGHMForm(formDataToSend);
    ...
    <div className="form-container">
      <form onSubmit={handleSubmit}>
        <div className="form-section">
          <h2 className="form-title">GHM Cutting Process Feedback</h2>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              <label>MLN Machine No</label>
              <input
                type="text"
                value={formData.mln_machine_no}
                onChange={(e) => handleChange('mln_machine_no', e.target.value)}
                className={errors.mln_machine_no ? 'error' : ''}
              />
              {errors.mln_machine_no && <div className="error-message">{errors.mln_machine_no}</div>}
            </div>
          </div>
